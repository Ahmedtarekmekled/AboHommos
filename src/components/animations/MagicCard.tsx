...
export function MagicCard(props: MagicCardProps) {
  const {
    ...
    className,
    gradientSize = 250,
    gradientColor = "#e5e7eb", // Light gray for light mode sites
    gradientOpacity = 0.5,
    gradientFrom = "rgba(251, 191, 36, 0.2)", // Amber matching site primary
    gradientTo = "rgba(217, 119, 6, 0.2)",
    mode = "gradient",
  } = props

  const glowFrom = isOrbMode(props) ? (props.glowFrom ?? "#fbbf24") : "#fbbf24"
  const glowTo = isOrbMode(props) ? (props.glowTo ?? "#d97706") : "#d97706"
  const glowAngle = isOrbMode(props) ? (props.glowAngle ?? 90) : 90
  const glowSize = isOrbMode(props) ? (props.glowSize ?? 420) : 420
  const glowBlur = isOrbMode(props) ? (props.glowBlur ?? 60) : 60
  const glowOpacity = isOrbMode(props) ? (props.glowOpacity ?? 0.8) : 0.8
  ...
  return (
    <motion.div
      className={cn(
        "group relative isolate overflow-hidden rounded-[inherit] border",
        className
      )}
